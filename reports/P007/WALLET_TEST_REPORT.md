# Velyxora Exchange — P007 Wallet Core & Custody Test Report
...
* `packages/blockchain`: Tests HD Wallet BIP-39 mnemonic, child paths derivation, standard `secp256k1` signature generation/verification across 8 adapters.
...
* `packages/wallet`: Tests multi-dimensional balances, operational limits, cold wallet restrictions, treasury multi-stage authorizations, and institutional custody integration.
* `packages/custody`: Tests vault provisioning, balances, 4-eyes approval workflows, and emergency locking/unlocking.
* `apps/wallet-service`: Tests PersistentWalletService bootstrap, allocation, updates, and double-entry trade settle integration.
* `apps/backend`: Tests public REST endpoints including new digital custody endpoints (/custody/overview, /custody/vaults, /custody/transfer, /custody/approve/:id, /custody/distribution, /custody/audit, and /custody/emergency/:action).
* `tests`: Runs heavy multi-threaded concurrency lock checks, custody integration, and stress tests.
...
ok  	velyxora/packages/custody	0.005s
